...
for i in 0 1 2 3 4 5 6 7 8; do
   # [ "$i" = "$used" ] && continue
   mkfs -t ext4 /dev/nvme3n1p${j}
   mkdir /scratch${i}
   mount -o rw,noatime,nodiratime,block_validity,delalloc,nojournal_checksum,barrier,user_xattr,acl /dev/nvme3n1p${j} /scratch${i}
   mkdir /scratch${i}/kvell
   sudo chown jyj:jyj /scratch${i}/kvell
   j=$((j+1))
done
